...
``current-window-scale`` (RW)
    ...
    option. It does not change its value unless the user specifically changes
    the value of the option. ``window-scale`` does not pay attention to
    realtime window changes and only updates if the user changes its value.
